...
class MockStackProvider extends StackProvider
{
    ...
    {
        $this->insertCards($stack, LoveLetter::PRINCESSCARD, 11);
        $this->insertCards($stack, LoveLetter::PRINCECARD, 1);
    }

    protected function princeNormalStack(&$stack)
    {
        $this->insertCards($stack, LoveLetter::KINGCARD, 10);
        $this->insertCards($stack, LoveLetter::BARONCARD, 1);
        $this->insertCards($stack, LoveLetter::PRINCECARD, 1);
    }
    ...
    {
        $this->insertCards($stack, LoveLetter::BARONCARD, 5);
        $this->insertCards($stack, LoveLetter::PRINCECARD, 1);
        $this->insertCards($stack, LoveLetter::KINGCARD, 1);
    }

    protected function countessStack(&$stack)
    {
        $this->insertCards($stack, LoveLetter::PRINCECARD, 7);
        $this->insertCards($stack, LoveLetter::COUNTESSCARD, 1);
    }
}
